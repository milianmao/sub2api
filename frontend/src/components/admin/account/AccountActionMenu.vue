        ...
        <div class="py-1">
          <template v-if="account">
            ...
            <template v-if="(account.type === 'oauth' || account.type === 'setup-token') && !isShadow">
              ...
              <button @click="$emit('copy-access-token', account); $emit('close')" class="flex w-full items-center gap-2 px-4 py-2 text-sm text-cyan-600 hover:bg-gray-100 dark:hover:bg-dark-700">
            ...
            <button v-if="isOpenAIOAuth" @click="$emit('generate-checkout-link', account); $emit('close')" class="flex w-full items-center gap-2 px-4 py-2 text-sm text-sky-600 hover:bg-gray-100 dark:hover:bg-dark-700">
              <Icon name="creditCard" size="sm" />
              {{ t('admin.accounts.generateCheckoutLink') }}
            </button>
...
const emit = defineEmits(['close', 'test', 'stats', 'schedule', 'duplicate', 'reauth', 'refresh-token', 'copy-access-token', 'recover-state', 'reset-quota', 'set-privacy', 'generate-checkout-link', 'create-spark-shadow'])
const { t } = useI18n()
