      ...
      <form id="key-form" @submit.prevent="handleSubmit" class="space-y-5">
        ...
        <div>
          <GroupSelector
            ...
            :groups="showEditModal ? groups : groups.filter((group) => getKeyGroupProvider(group.platform) === createProvider)"
...
const selectCreateProvider = (provider: KeyGroupProvider) => {
  ...
  const allowed = new Set(
    groups.value
      .filter((group) => getKeyGroupProvider(group.platform) === provider)
      .map((group) => group.id)
  )
  formData.value.group_ids = formData.value.group_ids.filter((id) => allowed.has(id))
}
