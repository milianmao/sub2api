...
type GroupQuery struct {
	...
	withAuthorizedAPIKeys *APIKeyQuery
...
// QueryAuthorizedAPIKeys chains the current query on the "authorized_api_keys" edge.
func (_q *GroupQuery) QueryAuthorizedAPIKeys() *APIKeyQuery {
	query := (&APIKeyClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *GroupQuery) Clone() *GroupQuery {
	...
	return &GroupQuery{
		...
		withAuthorizedAPIKeys: _q.withAuthorizedAPIKeys.Clone(),
...
// WithAuthorizedAPIKeys tells the query-builder to eager-load the nodes that are connected to
// the "authorized_api_keys" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *GroupQuery) WithAuthorizedAPIKeys(opts ...func(*APIKeyQuery)) *GroupQuery {
	query := (&APIKeyClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withAuthorizedAPIKeys = query
	return _q
}
...
func (_q *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := _q.withAuthorizedAPIKeys; query != nil {
		if err := _q.loadAuthorizedAPIKeys(ctx, query, nodes,
			func(n *Group) { n.Edges.AuthorizedAPIKeys = []*APIKey{} },
			func(n *Group, e *APIKey) { n.Edges.AuthorizedAPIKeys = append(n.Edges.AuthorizedAPIKeys, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *GroupQuery) loadAuthorizedAPIKeys(ctx context.Context, query *APIKeyQuery, nodes []*Group, init func(*Group), assign func(*Group, *APIKey)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int64]*Group)
	nids := make(map[int64]map[*Group]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(group.AuthorizedAPIKeysTable)
		s.Join(joinT).On(s.C(apikey.FieldID), joinT.C(group.AuthorizedAPIKeysPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(group.AuthorizedAPIKeysPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(group.AuthorizedAPIKeysPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "authorized_api_keys" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
