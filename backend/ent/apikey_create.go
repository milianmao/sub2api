...
func (_c *APIKeyCreate) AddAuthorizedGroupIDs(ids ...int64) *APIKeyCreate {
	_c.mutation.AddAuthorizedGroupIDs(ids...)
	return _c
}

// AddAuthorizedGroups adds the "authorized_groups" edges to the Group entity.
func (_c *APIKeyCreate) AddAuthorizedGroups(v ...*Group) *APIKeyCreate {
	ids := make([]int64, len(v))
	for i := range v {
		ids[i] = v[i].ID
	}
	return _c.AddAuthorizedGroupIDs(ids...)
}
...
func (_c *APIKeyCreate) createSpec() (*APIKey, *sqlgraph.CreateSpec) {
	...
	if nodes := _c.mutation.AuthorizedGroupsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: false,
