...
func (_q *APIKeyQuery) QueryAuthorizedGroups() *GroupQuery {
	query := (&GroupClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *APIKeyQuery) WithAuthorizedGroups(opts ...func(*GroupQuery)) *APIKeyQuery {
	query := (&GroupClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *APIKeyQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*APIKey, error) {
	...
	if query := _q.withAuthorizedGroups; query != nil {
		if err := _q.loadAuthorizedGroups(ctx, query, nodes,
			func(n *APIKey) { n.Edges.AuthorizedGroups = []*Group{} },
			func(n *APIKey, e *Group) { n.Edges.AuthorizedGroups = append(n.Edges.AuthorizedGroups, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *APIKeyQuery) loadAuthorizedGroups(ctx context.Context, query *GroupQuery, nodes []*APIKey, init func(*APIKey), assign func(*APIKey, *Group)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(apikey.AuthorizedGroupsTable)
		s.Join(joinT).On(s.C(group.FieldID), joinT.C(apikey.AuthorizedGroupsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(apikey.AuthorizedGroupsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(apikey.AuthorizedGroupsPrimaryKey[0]))
