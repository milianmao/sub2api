...
func (s *OpenAIGatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, body []byte) (*OpenAIForwardResult, error) {
	...
	if account.Type == AccountTypeOAuth && !compatMessagesBridge {
		origPromptCacheKey := promptCacheKey
		if c != nil {
			if value, ok := c.Get("openai_guard_prompt_cache_key_orig"); ok {
				if originalKey, ok := value.(string); ok && originalKey != "" {
					origPromptCacheKey = originalKey
				}
			}
		}
		body = s.applyCodexReasoningReplayCache(originalModel, origPromptCacheKey, body)
		requestView = newOpenAIRequestView(body)
		promptCacheKey = requestView.PromptCacheKey
		reqBody = nil
	}
		...
		if resp.StatusCode >= 400 {
			...
			if !httpInvalidEncryptedContentRetryTried && resp.StatusCode == http.StatusBadRequest && upstreamCode == "invalid_encrypted_content" {
				...
				if trimOpenAIEncryptedReasoningItems(decoded) {
					...
					if c != nil {
						if value, ok := c.Get("openai_guard_prompt_cache_key_orig"); ok {
							originalPromptCacheKey, _ = value.(string)
						}
					}
					s.clearCodexReasoningReplayOnInvalidSig(ctx, originalModel, originalPromptCacheKey)
					continue
				}
...
func (s *OpenAIGatewayService) buildUpstreamRequest(ctx context.Context, c *gin.Context, account *Account, body []byte, token string, isStream bool, promptCacheKey string, isCodexCLI bool) (*http.Request, error) {
	...
	if account.UsesOpenAICodexProtocol() {
		...
		req.Header.Set("x-codex-installation-id", isolateOpenAIUpstreamSessionID(0, account, "installation:"+promptCacheKey))
		if promptCacheKey != "" {
			identitySession := isolateOpenAIUpstreamSessionID(0, account, promptCacheKey)
			req.Header.Set("session-id", identitySession)
			req.Header.Set("thread-id", identitySession)
			req.Header.Set("x-client-request-id", identitySession)
			req.Header.Set("x-codex-window-id", identitySession+":0")
			rewriteCodexTurnMetadataFields(req.Header, map[string]any{
				"installation_id": req.Header.Get("x-codex-installation-id"),
				"session_id": identitySession,
				"thread_id": identitySession,
				"window_id": identitySession + ":0",
			})
		}
	}
	...
	if applyGuardSessionHeaders {
		...
		if c != nil {
			if value, ok := c.Get("openai_guard_confuse_state"); ok {
				confuseState, _ = value.(*guard.ConfuseState)
			}
		}
...
func (s *OpenAIGatewayService) clearCodexReasoningReplayOnInvalidSig(_ context.Context, originalModel, promptCacheKey string) {
...
func (s *OpenAIGatewayService) restoreOpenAIConfuseStreamData(c *gin.Context, data []byte) []byte {
	...
	value, ok := c.Get("openai_guard_confuse_state")
	if !ok {
		return data
	}
	state, ok := value.(*guard.ConfuseState)
	if !ok || state == nil {
		return data
	}
