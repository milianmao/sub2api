...
func adminRolePtr(role string) *string {
	return &role
}
...
func TestAdminService_UpdateUser_PromoteToAdmin(t *testing.T) {
	...
	updated, err := svc.UpdateUser(context.Background(), 42, &UpdateUserInput{Role: adminRolePtr(RoleAdmin)})
	require.NoError(t, err)
...
func TestAdminService_UpdateUser_InvalidRoleRejected(t *testing.T) {
	...
	_, err := svc.UpdateUser(context.Background(), 42, &UpdateUserInput{Role: adminRolePtr("root")})
	require.Error(t, err)
...
func (s *roleGuardUserRepoStub) ListWithFilters(_ context.Context, _ pagination.PaginationParams, filters UserListFilters) ([]User, *pagination.PaginationResult, error) {
	s.listCalls++
	if filters.Role == RoleSuperAdmin {
...
func TestAdminService_UpdateUser_DemoteLastAdminRejected(t *testing.T) {
	...
	_, err := svc.UpdateUser(context.Background(), 42, &UpdateUserInput{Role: adminRolePtr(RoleUser)})
	require.Error(t, err)
	require.Contains(t, err.Error(), "last admin")
	require.Nil(t, repo.lastUpdated, "最后一个管理员不应被降级持久化")
	require.Equal(t, 2, repo.listCalls, "降级路径应同时统计 admin 和 super_admin")
}
...
func TestAdminService_UpdateUser_DemoteLastSuperAdminRejected(t *testing.T) {
	...
	_, err := svc.UpdateUser(context.Background(), 42, &UpdateUserInput{Role: adminRolePtr(RoleAdmin)})
	require.Error(t, err)
...
func TestAdminService_UpdateUser_DemoteSuperAdminAllowedWhenAnotherExists(t *testing.T) {
	base := &userRepoStub{user: &User{ID: 42, Email: "root@example.com", Role: RoleSuperAdmin}}
	repo := &roleGuardUserRepoStub{
		rpmUserRepoStub: &rpmUserRepoStub{userRepoStub: base},
		superAdminTotal: 2,
...
func TestAdminService_UpdateUser_DemoteAdminAllowedWhenOthersExist(t *testing.T) {
	...
	updated, err := svc.UpdateUser(context.Background(), 42, &UpdateUserInput{Role: adminRolePtr(RoleUser)})
	require.NoError(t, err)
