...
func (s *OpenAIGatewayService) handleNonStreamingResponse(ctx context.Context, resp *http.Response, c *gin.Context, account *Account, originalModel, mappedModel string) (*openaiNonStreamingResult, error) {
	...
	if account != nil && account.Type == AccountTypeOAuth {
		body = s.restoreOpenAIConfuseStreamData(c, body)
	}
...
func (s *OpenAIGatewayService) handleSSEToJSON(resp *http.Response, c *gin.Context, body []byte, originalModel, mappedModel string) (*openaiNonStreamingResult, error) {
	...
	if ok {
		...
		if value, ok := c.Get("openai_guard_prompt_cache_key_orig"); ok {
			if promptCacheKey, ok := value.(string); ok && promptCacheKey != "" {
				s.cacheCodexReasoningReplayFromCompleted(originalModel, promptCacheKey, body)
			}
		}
	} else {
