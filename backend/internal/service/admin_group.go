...
func (s *adminServiceImpl) CreateGroup(ctx context.Context, input *CreateGroupInput) (*Group, error) {
	...
	openAIImageUpstream, err := NormalizeOpenAIImageUpstreamStrategy(input.OpenAIImageUpstream)
	if err != nil {
		return nil, err
	}
	visibleUserIDs := dedupePositiveInt64(input.VisibleUserIDs)
	...
	group := &Group{
		...
		AllowOpenAICompat:               input.AllowOpenAICompat,
		RequireOAuthOnly:                input.RequireOAuthOnly,
		RequirePrivacySet:               input.RequirePrivacySet,
		DefaultMappedModel:              input.DefaultMappedModel,
		MessagesDispatchModelConfig:     normalizeOpenAIMessagesDispatchModelConfig(input.MessagesDispatchModelConfig),
		OpenAIImageUpstream:             openAIImageUpstream,
	...
	if len(visibleUserIDs) > 0 {
		if err := s.syncGroupVisibleUsers(ctx, group.ID, visibleUserIDs); err != nil {
			return nil, err
		}
	}
...
func (s *adminServiceImpl) UpdateGroup(ctx context.Context, id int64, input *UpdateGroupInput) (*Group, error) {
	...
	if input.AllowOpenAICompat != nil {
		group.AllowOpenAICompat = *input.AllowOpenAICompat
	}
	...
	if input.OpenAIImageUpstream != nil {
		openAIImageUpstream, err := NormalizeOpenAIImageUpstreamStrategy(*input.OpenAIImageUpstream)
		if err != nil {
			return nil, err
		}
		group.OpenAIImageUpstream = openAIImageUpstream
	}
	...
	if input.VisibleUserIDs != nil {
		if err := s.syncGroupVisibleUsers(ctx, id, group.VisibleUserIDs); err != nil {
			return nil, err
		}
	}
	...
	if s.authCacheInvalidator != nil {
		...
		if input.VisibleUserIDs != nil {
			affectedUserIDs := append(oldVisibleUserIDs, group.VisibleUserIDs...)
...
func (s *adminServiceImpl) AdminUpdateAPIKeyGroup(ctx context.Context, keyID int64, req AdminUpdateAPIKeyGroupRequest) (*AdminUpdateAPIKeyGroupIDResult, error) {
	if req.GroupID == nil {
		return s.adminUpdateAPIKeyGroupID(ctx, keyID, nil)
	}
	if *req.GroupID < 0 {
		return nil, infraerrors.BadRequest("INVALID_GROUP_ID", "group_id must be non-negative")
	}

	authorizedGroupIDs := make([]int64, 0, len(req.GroupIDs)+1)
	seen := make(map[int64]struct{}, len(req.GroupIDs)+1)
	for _, groupID := range req.GroupIDs {
		if groupID <= 0 {
			return nil, infraerrors.BadRequest("INVALID_GROUP_ID", "group_ids must contain positive group IDs")
		}
		if _, ok := seen[groupID]; ok {
			continue
		}
		seen[groupID] = struct{}{}
		authorizedGroupIDs = append(authorizedGroupIDs, groupID)
	}
	if *req.GroupID > 0 {
		if _, ok := seen[*req.GroupID]; !ok {
			authorizedGroupIDs = append([]int64{*req.GroupID}, authorizedGroupIDs...)
		}
	}
	...
	loadOwner := func() (*User, error) {
		...
		loaded, loadErr := s.userRepo.GetByID(ctx, apiKey.UserID)
		if loadErr != nil {
			return nil, loadErr
		}
		owner = loaded
	...
	groups := make(map[int64]*Group, len(authorizedGroupIDs))
	restrictedGroups := make([]*Group, 0, len(authorizedGroupIDs))
	for _, groupID := range authorizedGroupIDs {
		...
		if group.IsSubscriptionType() {
			...
			if _, subErr := s.userSubRepo.GetActiveByUserIDAndGroupID(ctx, apiKey.UserID, groupID); subErr != nil {
	...
	if len(restrictedGroups) == 0 {
		...
		grantedGroupID := restrictedGroups[0].ID
		result.GrantedGroupID = &grantedGroupID
		result.GrantedGroupName = restrictedGroups[0].Name
	}
