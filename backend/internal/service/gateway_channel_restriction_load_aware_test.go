...
func TestSelectAccountWithLoadAwareness_UpstreamRestrictionRejectsWhenAllAccountsDisallowed(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"load-aware", true, false},
		{"legacy-mixed", false, false},
		{"legacy-single", false, true},
	} {
		t.Run(tc.name, func(t *testing.T) {
			f := newLoadAwareRestrictionFixture(t, true, nil, nil)
			f.svc.cfg.Gateway.Scheduling.LoadBatchEnabled = tc.loadAware
			if tc.forcePlatform {
				f.ctx = context.WithValue(f.ctx, ctxkey.ForcePlatform, PlatformAnthropic)
			}
			...
			require.Equal(t, 0, f.concurrencyCache.acquireAccountCalls)
		})
	}
}

func TestSelectAccountWithLoadAwareness_UpstreamRestrictionAllowsEligibleFallback(t *testing.T) {
	t.Parallel()
	f := newLoadAwareRestrictionFixture(t, true, nil, nil)
	f.svc.accountRepo.(*mockAccountRepoForPlatform).accounts[1].IsFallback = true
	result, err := f.svc.SelectAccountWithLoadAwareness(f.ctx, &f.groupID, "", "claude-fable-5-1", nil, "", 0)
	require.NoError(t, err)
	require.NotNil(t, result)
	require.NotNil(t, result.Account)
	require.Equal(t, int64(2), result.Account.ID)
	require.True(t, result.Account.IsFallback)
}
