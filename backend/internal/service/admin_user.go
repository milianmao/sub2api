...
func (s *adminServiceImpl) ensureNotLastAdmin(ctx context.Context) error {
	noSubs := false
	_, admins, err := s.userRepo.ListWithFilters(ctx,
	...
	_, superAdmins, err := s.userRepo.ListWithFilters(ctx,
		pagination.PaginationParams{Page: 1, PageSize: 1},
	...
	if admins == nil || superAdmins == nil || admins.Total+superAdmins.Total <= 1 {
...
func (s *adminServiceImpl) ensureNotLastSuperAdmin(ctx context.Context) error {
	...
	if superAdmins == nil || superAdmins.Total <= 1 {
...
func (s *adminServiceImpl) UpdateUser(ctx context.Context, id int64, input *UpdateUserInput) (*User, error) {
	...
	if input.Role != nil {
		...
		if user.IsSuperAdmin() && role != RoleSuperAdmin {
			if err := s.ensureNotLastSuperAdmin(ctx); err != nil {
				return nil, err
			}
		} else if user.IsAdmin() && !IsAdminRole(role) {
	...
	if s.authCacheInvalidator != nil {
		...
		if user.Concurrency != oldConcurrency || user.Status != oldStatus || user.Role != oldRole || user.RPMLimit != oldRPMLimit || user.Level != oldLevel || user.RestrictPublicGroups != oldRestrictPublicGroups || !sameInt64Set(user.AllowedGroups, oldAllowedGroups) {
