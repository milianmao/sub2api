...
func (s *UsageLogRepoSuite) TestDashboardStats_TodayTotalsAndPerformance() {
	...
	s.Require().Equal(baseStats.EnabledAccounts+4, stats.EnabledAccounts, "EnabledAccounts mismatch")
	s.Require().Equal(baseStats.SchedulableAccounts+3, stats.SchedulableAccounts, "SchedulableAccounts mismatch")
	s.Require().Equal(baseStats.ErrorAccounts+1, stats.ErrorAccounts, "ErrorAccounts mismatch")
	s.Require().Equal(baseStats.DisabledAccounts+1, stats.DisabledAccounts, "DisabledAccounts mismatch")
	...
	s.Require().Equal(stats.TotalAccounts, stats.EnabledAccounts+stats.ErrorAccounts+stats.DisabledAccounts, "account status buckets should sum to total")
