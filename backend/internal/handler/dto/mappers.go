...
func APIKeyFromService(k *service.APIKey) *APIKey {
	...
	out := &APIKey{
		...
		Groups:             GroupsFromServiceShallow(k.Groups),
	}
...
func GroupsFromServiceShallow(groups []*service.Group) []*Group {
	if len(groups) == 0 {
		return nil
	}
...
func GroupFromServiceAdmin(g *service.Group) *AdminGroup {
	...
	out := &AdminGroup{
		...
		VisibleUserIDs:              g.VisibleUserIDs,
...
func groupFromServiceBase(g *service.Group) Group {
	return Group{
		...
		AccessMode:                      g.EffectiveAccessMode(),
