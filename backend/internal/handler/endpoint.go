...
const (
	...
	EndpointImagesVariations     = "/v1/images/variations"
...
func NormalizeInboundEndpoint(path string) string {
	...
	case strings.Contains(path, EndpointImagesVariations) || strings.Contains(path, "/images/variations"):
		return EndpointImagesVariations
...
func DeriveUpstreamEndpoint(inbound, rawRequestPath, platform string) string {
	...
	case service.PlatformOpenAI, service.PlatformGrok:
		if inbound == EndpointEmbeddings || inbound == EndpointAlphaSearch || inbound == EndpointResponsesInputTokens || inbound == EndpointImagesGenerations || inbound == EndpointImagesEdits || inbound == EndpointImagesVariations || inbound == EndpointVideosGenerations || inbound == EndpointVideosEdits || inbound == EndpointVideosExtensions || inbound == EndpointVideos {
			return inbound
		}
