...
func TestUserHandlerCreateAdminCannotSetLevelOrAllowedGroups(t *testing.T) {
	svc := newStubAdminService()
	router := setupGroupAuthorizationAdminRouter(svc, service.RoleAdmin)
	body := `{"email":"new@example.com","password":"pass123","level":3,"allowed_groups":[10]}`
...
func TestUserHandlerUpdateAdminCannotSetLevelOrAllowedGroups(t *testing.T) {
	svc := newStubAdminService()
	router := setupGroupAuthorizationAdminRouter(svc, service.RoleAdmin)
	body := `{"level":4,"allowed_groups":[30]}`
